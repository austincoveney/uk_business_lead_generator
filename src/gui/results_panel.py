...
from PySide6.QtCore import (
    Qt, Signal, Slot, QSettings, QSortFilterProxyModel,
...
class BusinessTableModel(QAbstractTableModel):
    ...
    def __init__(self, parent=None):
        ...
        self.businesses = []
        self.headers = [
            "Priority", "Name", "Business Type", "Phone", 
            "Website", "Address", "Issues"
        ]
...
class ResultsPanel(QWidget):
    ...
    def setup_ui(self):
        ...
        self.table_view.horizontalHeader().setSectionResizeMode(1, QHeaderView.Stretch)  # Name column stretches
        self.table_view.horizontalHeader().setSectionResizeMode(5, QHeaderView.Stretch)  # Address column stretches
    ...
    def save_settings(self):
        ...
        self.settings.setValue("results/priority_filter", self.priority_combo.currentIndex())
    ...
    def load_results(self):
        ...
        # Update status
        self.result_count_label.setText(f"Found {len(businesses)} businesses in {location}")
    ...
    def show_business_details(self, business):
        ...
        self.performance_progress.setValue(int(business.get('performance_score', 0) or 0))
        self.seo_progress.setValue(int(business.get('seo_score', 0) or 0))
        self.accessibility_progress.setValue(int(business.get('accessibility_score', 0) or 0))
        self.best_practices_progress.setValue(int(business.get('best_practices_score', 0) or 0))
        
        # Issues
        issues = business.get('issues', [])
        if isinstance(issues, list) and issues:
            self.issues_text.setHtml("<ul>" + "".join([f"<li>{issue}</li>" for issue in issues]) + "</ul>")
        else:
    ...
    def on_priority_changed(self, index):
        ...
        # Update filter proxy model
        if index == 0:  # All
            self.proxy_model.setFilterFixedString("")
        elif index == 1:  # High
            # Custom filtering for priority == 1
            class PriorityProxyModel(QSortFilterProxyModel):
                def filterAcceptsRow(self, source_row, source_parent):
                    index = self.sourceModel().index(source_row, 0, source_parent)
                    business = self.sourceModel().data(index, Qt.UserRole)
                    return business.get('priority', 0) == 1
            
            self.proxy_model = PriorityProxyModel()
            self.proxy_model.setSourceModel(self.model)
            self.table_view.setModel(self.proxy_model)
        elif index == 2:  # Medium
            # Custom filtering for priority == 2
            pass
        elif index == 3:  # Low
            # Custom filtering for priority == 3
            pass
    ...
    def on_save_notes(self):
        ...
        success = self.current_database.update_business(business_id, {
            'notes': notes
        })
    ...
    def on_delete_business(self):
        ...
        if success:
            ...
            QMessageBox.information(self, "Deleted", "Business has been deleted successfully.")
        else:
    ...
    def on_export_csv(self):
        ...
        if count > 0:
            QMessageBox.information(
                self,
                "Export Successful",
                f"Exported {count} businesses to {file_path}"
            )
        else:
    ...
    def on_generate_report(self):
        ...
        if count > 0:
            ...
            QMessageBox.warning(self, "Report Generation Failed", "Failed to generate report.")