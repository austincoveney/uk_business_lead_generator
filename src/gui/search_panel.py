...
class SearchPanel(QWidget):
    ...
    def load_settings(self):
        ...
        self.analyze_websites_checkbox.setChecked(self.settings.value("search/analyze_websites", True, bool))
    ...
    def save_settings(self):
        ...
        self.settings.setValue("search/last_category_index", self.category_input.currentIndex())
    ...
    def on_search_clicked(self):
        ...
        if not validate_uk_location(location):
            QMessageBox.warning(
                self, 
                "Invalid Location", 
                "Please enter a valid UK location (city, town, or postal code)."
            )
            return
    ...
    def perform_search(self, location, category, limit, analyze_websites, priority_focus):
        ...
        try:
            ...
            for i, business in enumerate(businesses):
                ...
                self.update_status(f"Processing {business['name']} ({i+1}/{len(businesses)})...", progress)
                ...
                if 'website' not in business or not business['website']:
                    ...
                    # Default before analysis
                    business['priority'] = 2  # Assumes poor website until proven otherwise
                
                # Analyze website if requested and available
                if analyze_websites and 'website' in business and business['website']:
                    try:
                        # Analyze website
                        analysis_results = analyzer.analyze_website(business['website'])
                        
                        # Update business with analysis results
                        business.update(analysis_results)
                        
                        # Update priority based on priority focus
                        if priority_focus == "no_website" and business['priority'] != 1:
                            # Skip this business
                            continue
                        elif priority_focus == "poor_website" and business['priority'] != 2:
                            # Skip this business
                            continue
            ...
            self.update_status(f"Search completed. Found {len(businesses)} businesses.", 100)
